...
class CombinedTreeTransformer(BaseTransformer):
    ...
    def transform(self, t, *args):
        t = self.const_tree_transformer.transform(t)
        for u, d in t.nodes(data=True):
            del d['type']

        dep_t = self.__remove_dep_edges__(args[0])

        const_leaves = sorted([(i, d) for i, d in t.nodes(data=True) if 'token_id' in d], key=lambda x: x[1]['token_id'])
        assert (t.number_of_nodes() - len(const_leaves)) == dep_t.number_of_edges()-1

        new_bin_t = t.copy(as_view=False)
        ...
        for u, v, d in dep_t.edges(data=True):
            ...
            assert id_token_u == t.nodes[u_in_const]['token_id']
            assert id_token_v == t.nodes[v_in_const]['token_id']

            assert dep_t.nodes[u]['word'] == t.nodes[u_in_const]['word']
            assert dep_t.nodes[v]['word'] == t.nodes[v_in_const]['word']

            if dep_t.in_degree(u) == 0:
                # u is a leaf
                lca = list(t.successors(u_in_const))[0]
            else:
