...
class ParamListTrainer(Experiment):
    ...
    def save_checkpoint(self, config):
        ...
        with open(os.path.join(self.log_dir,'checkpoint.pickle'), 'wb') as f:
            pickle.dump(d, f)
        # json_save_dict(d, self.log_dir + 'checkpoint.json')
        with open(os.path.join(self.log_dir,'checkpoint.json'), 'w') as f:
    ...
    def foo(self, config=None):
        ...
        for i, params in enumerate(self.param_list[start_i:]):
            ...
            res = self.train_foo(id=i, log_dir=train_log_dir, params=params)
            ...
            self.experiment_log.info(s)

    def load_checkpoint(self):
        checkpoint_f = os.path.join(self.log_dir,'checkpoint.pickle')
        if os.path.exists(checkpoint_f):
            with open(checkpoint_f, 'rb') as f:
                d = pickle.load(f)
                if 'results' in d:
