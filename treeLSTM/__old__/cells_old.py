...
# we assume size contains same len, i.e otuput has size n^d \time n.
# only n^d is symmentric.
# the output axis is the last one
...
class GenericTreeLSTMCell(nn.Module):
    ...
    def compute_iou_gate(self, neighbour_h, nodes):
    ...
    def reduce_func(self, nodes):
        ...
        iou_aggr = self.compute_iou_gate(neighbour_h, nodes)
...
class BinaryFullTensorCell(GenericTreeLSTMCell):
    ...
    def __init__(self, h_size, max_output_degree, pos_stationarity):
        ...
        if self.pos_stationarity:
            ...
            self.U = nn.Parameter(th.randn(h_size, 3*h_size))
            self.b = nn.Parameter(th.randn(3*h_size))
        else:
            # define parameters for the computation of iou
            self.A = nn.Parameter(th.randn(h_size, h_size, 3*h_size))
            self.U1 = nn.Parameter(th.randn(h_size, 3*h_size))
            self.U2 = nn.Parameter(th.randn(h_size, 3*h_size))
            self.b = nn.Parameter(th.randn(3*h_size))
...
class SumChildCell(GenericTreeLSTMCell):
    ...
    def __init__(self, h_size, max_output_degree, pos_stationarity):
        super(SumChildCell, self).__init__(h_size, max_output_degree, pos_stationarity)

        if not self.pos_stationarity:
            #NARY aggregation
            # define parameters for the computation of iou
            self.U = nn.Parameter(th.randn(max_output_degree * h_size, 3*h_size))
            self.b = nn.Parameter(th.randn(3*h_size))
        else:
        ...
        #self.b = nn.Parameter(th.zeros(3 * h_size), requires_grad=False)
    # neighbour_states has shape batch_size x n_neighbours x insize
    def compute_iou_gate(self, neighbour_h, nodes):
        if not self.pos_stationarity:
...
class CANCOMPCell(GenericTreeLSTMCell):
    ...
    def __init__(self, h_size, max_output_degree, rank, pos_stationarity, scale_factor):
        ...
        # NOT POSSIBLE: maybe is better using 3 x rank x hsize with batched matrix multiplication!
        self.Ui_output = nn.Parameter(th.randn((rank, h_size)))
        self.Uo_output = nn.Parameter(th.randn((rank, h_size)))
        self.Uu_output = nn.Parameter(th.randn((rank, h_size)))

        # attribute for the miz-max scaler
    ...
    def compute_iou_gate(self, neighbour_h, nodes):
        ...
        gate_i = th.sum(r_i.reshape((-1, self.rank, 1)) * self.Ui_output.reshape((1, self.rank, -1)), dim=1) / self.scale_factor
        gate_o = th.sum(r_o.reshape((-1, self.rank, 1)) * self.Uo_output.reshape((1, self.rank, -1)), dim=1) / self.scale_factor
        gate_u = th.sum(r_u.reshape((-1, self.rank, 1)) * self.Uu_output.reshape((1, self.rank, -1)), dim=1) / self.scale_factor
...
# IGNORE NEXT CLASS
# h3 =  Canonical decomposition
class CANCOMPCellInput(GenericTreeLSTMCell):

    def __init__(self, x_size, h_size, max_output_degree, rank, pos_stationarity):
        super(CANCOMPCellInput, self).__init__(h_size, max_output_degree, pos_stationarity)

        self.rank = rank
        print('USING INPUT IN DECOMPOSITION')
        self.U_input = nn.Parameter(th.randn((x_size, 3*rank)))
        self.B_input = nn.Parameter(th.randn(3 * rank))
    ...
    # neighbour_states has shape batch_size x n_neighbours x insize
    def compute_iou_gate_input(self, neighbour_h, x_input):

        ris = th.addmm(self.B_input, x_input, self.U_input)
        ...
        gate_i = th.sum(r_i.reshape((-1, self.rank, 1)) * self.Ui_output.reshape((1, self.rank, -1)), dim=1)
        gate_o = th.sum(r_o.reshape((-1, self.rank, 1)) * self.Uo_output.reshape((1, self.rank, -1)), dim=1)
        gate_u = th.sum(r_u.reshape((-1, self.rank, 1)) * self.Uu_output.reshape((1, self.rank, -1)), dim=1)
    ...
    def reduce_func(self, nodes):
        ...
        # add the input contribution
        f_aggr = self.compute_forget_gate(neighbour_h) + (nodes.data['f_input']).repeat((1, self.max_output_degree))
        iou_aggr = self.compute_iou_gate_input(neighbour_h, nodes.data['x'])
    ...
    def apply_node_func(self, nodes):
        ...
        if 'iou_aggr' in nodes.data:
            ...
            # leaves node
            ris = th.addmm(self.B_input, nodes.data['x'], self.U_input)
            (r_i, r_o, r_u) = th.chunk(ris, 3, 1)
            i = th.sum(r_i.reshape((-1, self.rank, 1)) * self.Ui_output.reshape((1, self.rank, -1)), dim=1)
            o = th.sum(r_o.reshape((-1, self.rank, 1)) * self.Uo_output.reshape((1, self.rank, -1)), dim=1)
            u = th.sum(r_u.reshape((-1, self.rank, 1)) * self.Uu_output.reshape((1, self.rank, -1)), dim=1)
...
class SumChildIOUInputDependentCell(GenericTreeLSTMCell):

    def __init__(self, h_size, max_output_degree, pos_stationarity):
        super(SumChildIOUInputDependentCell, self).__init__(h_size, max_output_degree, pos_stationarity)
...
class SumChildFInputDependentCell(SumChildCell):

    def __init__(self, h_size, max_output_degree, pos_stationarity):
        super(SumChildFInputDependentCell, self).__init__(h_size, max_output_degree, pos_stationarity)
...
class SumChildALLInputDependentCell(GenericTreeLSTMCell):

    def __init__(self, h_size, max_output_degree, pos_stationarity):
        super(SumChildALLInputDependentCell, self).__init__(h_size, max_output_degree, pos_stationarity)
