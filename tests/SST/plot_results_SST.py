...
with open('checkpoints/nary_{}_ModelSelection_mul/checkpoint.pickle'.format(ds_name), 'rb') as f:
    d = pickle.load(f)
ris = d['results']
vl_metric = np.array([x['vl_acc'] for x in ris]).reshape(size_ris)
tr_metric = np.array([x['tr_acc'] for x in ris]).reshape(size_ris)
...
with open('checkpoints/full_{}_ModelSelection_mul/checkpoint.pickle'.format(ds_name), 'rb') as f:
    d = pickle.load(f)
ris = d['results']
vl_metric = np.array([x['vl_acc'] for x in ris]).reshape(size_ris)
tr_metric = np.array([x['tr_acc'] for x in ris]).reshape(size_ris)
