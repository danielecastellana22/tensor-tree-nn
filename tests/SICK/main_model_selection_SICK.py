...
def get_train_and_validate_fun(args):
    ...
    def train_foo(id, log_dir, params):
        ...
        model = create_sick_model(trainset.num_vocabs,
                                  ...
                                  cell_type=args.cell_type, rank=params['rank']).to(device)
        ...
        optimizer = optim.Adagrad([{'params': params_ex_emb, 'lr': params['lr'], 'weight_decay': args.weight_decay}])

        # train and validate
        best_model, best_dev_metrics = train_and_validate(model, sick_extract_batch_data, sick_loss_function, optimizer,
                                                          trainset, devset, device,
